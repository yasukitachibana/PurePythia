...
void SetXML::OpenXMLFile()
{
  ...
  if (!xml_file_open)
  {
    ...
    if (xml_doc.ErrorID() < 1)
    {
      ...
      xml_root = (tinyxml2::XMLElement *)xml_doc.FirstChildElement("purePythia");
      if (!xml_root)
      {
        std::cout << std::endl;
...
//________________________________________________________________
std::vector<std::string> SetXML::GetElementVectorText(std::initializer_list<const char *> path,
                                                   bool isRequired /* = true */)
{
  ...
  if (element)
  {
    ...
    while (element != nullptr)
    {
      std::string iOutListValue = element->GetText();
      vecList.push_back(iOutListValue);
      element = element->NextSiblingElement(*pathEnd);
    }
    return vecList;
